...
def plot_series_and_doy(
    ...
    2 × 2 figure per logger:
      ...
      [0,1] ax2 – CORA monthly mean ± std + logger mean & median markers
                  [B1 FIX] f-string now correctly formats coordinates in title
                  [B2 FIX] panel shows "unavailable" message when cora_df is None
      [1,0] ax3 – DOY vs CORA interannual + MEAN+MEDIAN markers
                  [B2 FIX] CORA background skipped when cora_df is None
      [1,1] ax4 – WOD Monthly Climatology vs Logger
                  (unchanged from v.20260630)
    """
    ...
    # CORA pre-computations — only when available
    ...
    ax1.plot(
        ...
        alpha=0.35, linewidth=0.7, color="steelblue", label="Raw data",
    )
    ...
    # B1 FIX: was missing f"…" prefix so lat/lon were printed literally
    # B2 FIX: guard against cora_df being None
    if cora_available:
        ...
        # B1 FIX: added f-prefix so {latitude:.2f} and {longitude:.2f} are evaluated
        ax2.set_title(f"CORA Monthly Mean ± Std vs Logger | ({latitude:.2f}, {longitude:.2f})")
        ax2.legend(fontsize=8)
        ax2.grid(True, alpha=0.3)
    else:
        ax2.text(0.5, 0.5, "CORA Climatology\nUnavailable",
                 transform=ax2.transAxes, ha="center", va="center",
                 color="#c62828", fontsize=12, fontweight="bold")
        ax2.set_title(f"CORA Monthly Mean ± Std — N/A | ({latitude:.2f}, {longitude:.2f})")

    # ── shared CORA DOY background ─────────────────────────────────────────────
    def _draw_cora_doy(ax: plt.Axes) -> None:
        # B2 FIX: skip loop when cora_df is None
        if cora_available:
    ...
    no_cora_suffix = "" if cora_available else " [no CORA]"
    ax3.set_title(f"CORA Monthly Mean — DOY vs logger | ({latitude:.2f}, {longitude:.2f}){no_cora_suffix}")

    # ── [1,1] WOD Monthly Climatology vs Logger ───────────────────────────────
...
def plot_doy_all(
    ...
    """DOY scatter + all loggers (filled=mean, open=median). B2 FIX: cora_df may be None."""
    ...
    if cora_df is not None and not cora_df.empty:
        ...
        ax.text(0.5, 0.98, "CORA background unavailable", transform=ax.transAxes,
...
def build_report_pdf(
    ...
    """
    Multi-page landscape-A4 PDF.
    Structure: Cover → per-file 2×2 pages → DOY-all + monthly-all summary.
    B2 FIX: cora_df passed through as-is (may be None); plot functions handle it.
    """
    buf = io.BytesIO()
    ...
    title_style = ParagraphStyle("CS_Title", parent=styles["Title"],
                                 fontSize=20,
                                 textColor=rl_colors.HexColor("#00A6D6"), spaceAfter=6)
    h1_style    = ParagraphStyle("CS_H1", parent=styles["Heading1"],
                                 fontSize=13,
                                 textColor=rl_colors.HexColor("#00A6D6"),
                                 spaceBefore=10, spaceAfter=4)
    body_style  = ParagraphStyle("CS_Body",   parent=styles["Normal"], fontSize=9, spaceAfter=4)
    centre_style = ParagraphStyle("CS_Centre", parent=body_style,
    ...
    cora_note = "CORA (EMODnet-Physics ERDDAP, 1990–2023)" if cora_df is not None else "CORA unavailable"
    for line in [
        ...
        f"<b>Reference climatology:</b> {cora_note}",
    ]:
    ...
    story.append(Paragraph("CS-MACH1 Project · Ocean Temperature Monitoring Platform", centre_style))
    ...
    for fname, proc_df in logger_dfs.items():
        ...
        story.append(HRFlowable(width="100%", thickness=1, color=rl_colors.HexColor("#00A6D6")))
        story.append(Spacer(1, 0.3 * rl_cm))
        fig = plot_series_and_doy(clean_dfs[fname], proc_df, cora_df, latitude, longitude)
...
with st.sidebar:
    ...
    # PDF — only meaningful after processing
    st.markdown("### 📥 Export")
    if st.button("📄 Generate PDF Report", use_container_width=True):
        with st.spinner("Building PDF…"):
            try:
                _lat = float(next(iter(st.session_state["logger_dfs"].values()))["latitude"].iloc[0])
                _lon = float(next(iter(st.session_state["logger_dfs"].values()))["longitude"].iloc[0])
                pdf_bytes = build_report_pdf(
                    logger_dfs  = st.session_state["logger_dfs"],
                    clean_dfs   = st.session_state["clean_dfs"],
                    cora_df     = fetch_cora_data(_lat, _lon),
                    latitude    = _lat,
                    longitude   = _lon,
...
if start_button and "uploaded_files" in st.session_state:
    ...
    sidebar_status.caption(f"Parsed {len(clean_dfs)}/{total} file{'s' if total != 1 else ''} OK.")
    if not clean_dfs:
...
# ── Reactive re-processing on slider change ───────────────────────────────────
if (
...
if "logger_dfs" in st.session_state:
    ...
    # B2 FIX: was st.error() + st.stop() — now non-blocking warning
    if cora_df is None:
        st.warning(
            "⚠️ CORA climatology not available for this location "
            "(404 / network error). Continuing without CORA — "
            "plots will show logger data only."
        )
